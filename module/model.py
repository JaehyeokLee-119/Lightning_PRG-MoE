...
class Encoder_partial_freeze(nn.Module):
    ...
    def __init__(self, encoder_name, unfreeze=3):
        ...
        if (unfreeze != 0):
            ...
            for _, param in self.encoder.pooler.named_parameters():
                param.requires_grad = True
            for _, param in self.encoder.embeddings.named_parameters():
                param.requires_grad = True
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, return_dict=False):
        _, max_seq_len = input_ids.shape
        output_1, output_2 = self.encoder(input_ids=input_ids.view(-1, max_seq_len),
                                          attention_mask=attention_mask.view(-1, max_seq_len),
                                          token_type_ids=token_type_ids.view(-1, max_seq_len),
                                          return_dict=False)
        # self.encoder output을 2개로 나누기 (token단위, utterance 단위)
        return (output_1, output_2)

class PRG_MoE_parallel_gpu2(nn.Module):
    def __init__(self, dropout=0.5, n_speaker=2, n_emotion=7, n_cause=2, n_expert=2, guiding_lambda=0, **kwargs):
        super(PRG_MoE_parallel_gpu2, self).__init__()
        ...
        # Model
        # self.encoder = Encoder_partial_freeze(encoder_name=encoder_name, unfreeze=self.unfreeze) # For Unfreezing
        self.encoder = AutoModel.from_pretrained(encoder_name)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, speaker_ids):
        emotion_pred = self.emotion_classification_task(
    ...
    def get_encoder_output(self, input_ids, attention_mask, token_type_ids, max_seq_len):
        if 'bert-base' in self.encoder_name:
            first_output, pooled_output = self.encoder(input_ids=input_ids.view(-1, max_seq_len),
                                        attention_mask=attention_mask.view(-1, max_seq_len),
                                        token_type_ids=token_type_ids.view(-1, max_seq_len),
                                        return_dict=False)
        else:
        ...
        return first_output, pooled_output
    
    def emotion_classification_task(self, input_ids, attention_mask, token_type_ids):
        batch_size, max_doc_len, max_seq_len = input_ids.shape
        
        _, pooled_output = self.get_encoder_output(input_ids=input_ids.view(-1, max_seq_len),
                                    attention_mask=attention_mask.view(-1, max_seq_len),
                                    token_type_ids=token_type_ids.view(-1, max_seq_len),
...
class PRG_MoE_General(nn.Module):
    def __init__(self, dropout=0.5, n_speaker=2, n_emotion=7, n_cause=2, n_expert=2, guiding_lambda=0, **kwargs):
        ...
        # Model
        # self.encoder = Encoder_partial_freeze(encoder_name=encoder_name, unfreeze=self.unfreeze)
        self.encoder = AutoModel.from_pretrained(encoder_name)
        self.emotion_linear = nn.Linear(self.encoder.config.hidden_size, n_emotion)
        self.gating_network = nn.Linear(2 * (self.encoder.config.hidden_size + n_emotion + 1), n_expert)
        self.cause_linear = nn.ModuleList()
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, speaker_ids):
        emotion_pred = self.emotion_classification_task(
            input_ids, attention_mask, token_type_ids)
        cause_pred = self.binary_cause_classification_task(
            emotion_pred, input_ids, attention_mask, token_type_ids, speaker_ids)
    ...
    def binary_cause_classification_task(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        pair_embedding = self.get_pair_embedding(
            emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids)
        gating_prob = self.gating_network(
            pair_embedding.view(-1, pair_embedding.shape[-1]).detach())
        ...
        pred = []
        for _ in range(self.n_expert):
            expert_pred = self.cause_linear[_](
                pair_embedding.view(-1, pair_embedding.shape[-1]))
            expert_pred *= gating_prob.view(-1, self.n_expert)[:, _].unsqueeze(-1)
            pred.append(expert_pred)
    ...
    def get_pair_embedding(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        ...
        _, pooled_output = self.encoder(input_ids=input_ids.view(-1, max_seq_len), attention_mask=attention_mask.view(-1,
                                        max_seq_len), token_type_ids=token_type_ids.view(-1, max_seq_len), return_dict=False)
        utterance_representation = self.dropout(pooled_output)

        concatenated_embedding = torch.cat((utterance_representation, emotion_prediction, speaker_ids.view(
            -1).unsqueeze(1)), dim=1)  # 여기서 emotion_prediction에 detach를 해야 문제가 안생기겠지? 해보고 문제생기면 detach 고고
